...
if(canSeePlayer == false && canHearPlayer == false){    //This has to ultimately be the final else
	...
	if (target==player) {
		...
		if(distanceToPlayer >=5 && target==player){
			runSpeed=5;
			animation["run"].speed=2.5;
			}	
		if(distanceToPlayer<5 && distanceToPlayer>=4 && target==player){
			runSpeed=3;
			animation["run"].speed=1;
			}
		if(distanceToPlayer <4 && target==player){
			runSpeed=0;
			animation.CrossFade("idle");
			}
	...
	function Attack(){
	if(distanceToPlayer<5.5 && canAttack == true){
		...
		canHurt=true;
		if (distanceToPlayer<5.5 && canHurt==true){
		Hurt();
		//Debug.Log("Hit");
