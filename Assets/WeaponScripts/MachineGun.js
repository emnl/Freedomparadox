...
function Fire () {
	...
	if(nextFireTime < Time.time && bulletsLeft != 0 && reloading==false) {
		audio.clip = gunSounds[0];
		FireOneShot();
		transform.Find("sr3_finalmb").animation.Play("fire");
		nextFireTime += fireRate;
	}
	
	else if (nextFireTime < Time.time && bulletsLeft ==0 && reloading==false){
...
function FireOneShot () {
	...
	// Did we hit anything?
	//transform.parent.TransformDirection(Random.insideUnitSphere * recoil);
...
function Reload () {
	...
	reloading=true;
	transform.Find("sr3_finalmb").animation.Play("reload");
